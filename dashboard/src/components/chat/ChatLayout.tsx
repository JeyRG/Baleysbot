    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      {!isNavOpen && (
        <button 
          ...
          className="fixed bottom-6 left-6 w-12 h-12 bg-slate-900 text-white rounded-2xl flex items-center justify-center shadow-2xl z-50 hover:scale-110 transition-transform animate-in fade-in slide-in-from-left-4"
        >
      ...
      {/* Side Navigation */}
      <nav className={`bg-slate-900 flex flex-col py-8 z-40 transition-all duration-500 ease-in-out ${isNavOpen ? 'w-64 px-4' : 'w-0 opacity-0 -translate-x-full overflow-hidden'}`}>
        <div className="flex items-center gap-3 px-4 mb-10 shrink-0">
            ...
            <div className="flex flex-col">
                <span className="text-white font-black text-sm tracking-tight">Bot2703</span>
                <span className="text-blue-400 text-[10px] font-bold uppercase tracking-widest">Admin</span>
            </div>
            <button 
        ...
        <div className="space-y-2 flex-1">
            <button 
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 ${activeView === 'dashboard' ? 'bg-blue-600/10 text-blue-400 shadow-inner' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}
            >
            ...
            <button 
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all duration-300 ${activeView === 'inbox' ? 'bg-blue-600/10 text-blue-400 shadow-inner' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}
            >
                ...
                {supportCount > 0 && (
                    <span className="w-5 h-5 bg-rose-500 rounded-full flex items-center justify-center text-[10px] text-white font-black animate-pulse shadow-lg shadow-rose-500/20">
                        {supportCount}
                    </span>
                )}
            </button>
            ...
            <button 
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 ${activeView === 'training' ? 'bg-blue-600/10 text-blue-400 shadow-inner' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}
            >
            ...
            <button 
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 ${activeView === 'connection' ? 'bg-blue-600/10 text-blue-400 shadow-inner' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}
            >
        ...
        <button 
            ...
            className="w-full flex items-center gap-4 p-4 rounded-2xl text-slate-500 hover:text-rose-400 hover:bg-rose-400/5 transition-all duration-300 mt-auto"
        >
      ...
      <AnimatePresence mode="wait">
        {activeView === 'inbox' ? (
          ...
          >
              ...
              <aside className="w-1/3 max-w-[400px] min-w-[300px] bg-white border-r border-slate-200 flex flex-col shadow-xl z-30 transition-all duration-500 ease-in-out">
                  <div className="p-6 border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-10">
                      <div className="flex items-center justify-between">
                          <div>
                              <h1 className="font-black text-2xl text-slate-900 tracking-tight font-outfit">Inbox</h1>
                              <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest mt-1">Gestión de Leads UNAC</p>
                          </div>
                          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${
                              realtimeStatus === 'connected' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                              realtimeStatus === 'error' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                              'bg-amber-50 text-amber-600 border-amber-100'
                          }`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${realtimeStatus === 'connected' ? 'bg-emerald-500' : realtimeStatus === 'error' ? 'bg-rose-500' : 'bg-amber-500 animate-pulse'}`} />
              ...
              <main className="flex-1 flex flex-col bg-slate-50 relative overflow-hidden">
                  ...
                  <div className="m-auto flex flex-col items-center justify-center space-y-4 animate-in fade-in zoom-in duration-700">
                      <div className="w-24 h-24 bg-white rounded-[2rem] shadow-2xl flex items-center justify-center border border-slate-100 text-slate-200 animate-float">
                      ...
                      <div className="text-center">
                          <h3 className="text-lg font-bold text-slate-900 font-outfit">Bienvenido al Centro de Control</h3>
                          <p className="text-sm text-slate-400">Selecciona un chat en la izquierda para comenzar a gestionar.</p>
                      </div>
                  </div>
                  )}
              </main>
