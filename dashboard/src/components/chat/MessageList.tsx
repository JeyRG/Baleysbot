...
export function MessageList({ conversation, students }: MessageListProps) {
    ...
    return (
        <div className="flex flex-col h-full relative">
            {/* Header del Chat */}
            <div className="px-6 py-4 glass-panel/80 backdrop-blur-md border-b border-gray-100 flex justify-between items-center z-20 sticky top-0">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-border flex items-center justify-center font-bold text-gray-700 shadow-sm border border-border">
                ...
                <button 
                    ...
                    className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 shadow-sm border ${isHumanMode ? 'glass-panel text-rose-600 border-rose-100 hover:bg-rose-50 hover:border-rose-200' : 'bg-slate-900 text-white border-slate-900 hover:bg-slate-800 shadow-slate-900/10 hover:-translate-y-0.5'}`}
                >
                    ...
                    return (
                        <div key={msg.id} className={`flex ${isUser ? 'justify-start' : 'justify-end'} animate-in fade-in slide-in-from-bottom-2 duration-300 group`}>
                            <div className={`max-w-[80%] rounded-2xl p-4 shadow-sm border ${
                                isUser ? 'glass-panel text-foreground border-border rounded-tl-none' : 
                                        ...
                                        {msg.media_url.match(/\.(jpeg|jpg|gif|png|webp)/i) || msg.media_url.includes('image') ? (
                                            ...
                                            <a 
                                                ...
                                                className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                                                    isUser ? 'bg-secondary border-border hover:bg-border' : 'glass-panel/10 border-white/20 hover:glass-panel/20'
                                                }`}
                                            >
            ...
            {/* Input Area */}
            <div className="p-6 glass-panel border-t border-gray-100 z-10">
                <form onSubmit={sendMessage} className="flex gap-4 items-end">
                    <div className="relative flex-1">
                        <textarea
                            ...
                            className="w-full bg-secondary border-none rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-blue-500/20 transition-all min-h-[56px] max-h-32 resize-none"
                        />
                        <div className="absolute right-4 bottom-4 flex items-center gap-2">
                            ...
                            <button
                                ...
                                disabled={uploading}
                                className={`p-2 rounded-lg hover:bg-slate-200 transition-colors ${uploading ? 'animate-pulse text-blue-500' : 'text-muted-foreground'}`}
                            >
